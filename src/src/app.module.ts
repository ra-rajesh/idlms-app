...
@Module({
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: async (configService: ConfigService) => {
        // Safe default for missing NODE_ENV
        const env = process.env.NODE_ENV || 'development';

        // Load per-env config from ./config/index.ts
        const cfg = configService.get<any>(env);
        if (!cfg) {
          throw new Error(
            `Missing TypeORM config for env "${env}". Add a "${env}" block in src/config/index.ts`,
          );
        }

        // Honor DB_SSL=true (works without mounting RDS CA;
        // later you can set rejectUnauthorized: true with the CA)
        const sslEnabled = String(process.env.DB_SSL).toLowerCase() === 'true';
        const sslPatch = sslEnabled
  ...
  providers: [
    ...
    { provide: APP_GUARD, useClass: ApiClientPermissionsGuard },
  ],
})
